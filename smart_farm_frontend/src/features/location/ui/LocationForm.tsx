...
export const LocationForm: React.FC<{ toEditLocation?: Location, setClosed: React.Dispatch<React.SetStateAction<boolean>> }> = ({ toEditLocation, setClosed }) => {
    const auth = useAuth();
    ...
    useEffect(() => {
        if (toEditLocation) {
            ...
            setIsPublic(toEditLocation.isPublic);
    ...
    return (
        <>
            {!auth.isAuthenticated ? (
                ...
                <form onSubmit={(e) => {
                e.preventDefault()
                toEditLocation ? handleEdit(): handleSave()}
                }>
                <Grid gutter={"md"}>
                    {/*Name*/}
                    <Grid.Col span={12}>
